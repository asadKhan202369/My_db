...
import { asynctopRatedTV } from '@/store/actions/movieActions';
...
const index = () => {
  ...
  const { topReatedt,page} = useSelector((state) => state.movieReducer);
  ...
  console.log(topReatedt,"its movies");
  useEffect(() => {
     dispatch(asynctopRatedTV());    
  }, [page]);
 
  const getmoreposts =async ()=>{
    if(topReatedt.length >= 40){
      setTimeout(() => {
        // setShowMessage(true);
        dispatch(asynctopRatedTV()); 
